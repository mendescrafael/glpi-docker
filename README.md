...
## Projeto de conteinerização Docker
...
Consulte a seção [Estrutura de arquivos e diretórios do projeto](#estrutura-de-arquivos-e-diretórios-do-projeto) para obter mais detalhes sobre a estrutura de arquivos e diretórios adotada pelo projeto.
...
- Substituir as variáveis de ambiente nos [Arquivos `.template`](#arquivos-template);
...
> **Nota:** Afim de manter a organização do arquivo, ao adicionar novos alvos, crie-os no local indicado logo antes do alvo `help`, seguindo o padrão `nomeaplicacao-comando`, por exemplo, `glpi-clear-cache`.
...
### Componentes Docker Compose (`docker-compose.yml` e `docker-compose.dev.yml`)
...
## Especificações de versões dos artefatos
...
Veja mais detalhes na seção [Makefile](#makefile).
